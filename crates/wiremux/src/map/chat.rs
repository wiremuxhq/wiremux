...
fn encode_sampling(ir: &IrRequest, body: &mut Value, report: &mut LossReport) {
    ...
    if let Some(include_obfuscation) = s.include_obfuscation {
        ...
        report.record(
            ...
            LossAction::Preserve,
            "chat stream_options.include_obfuscation",
        );
    }
...
fn encode_prompt_cache_options(s: &IrSampling, body: &mut Value, report: &mut LossReport) {
    ...
    if let Some(mode) = mode {
        ...
        report.record(
            ...
            LossAction::Preserve,
            "chat prompt_cache_options.mode",
        );
    }
    if let Some(ttl) = ttl {
    ...
    if input.is_some() || output.is_some() {
        ...
        if let Some(mode) = input {
            ...
            report.record(
                ...
                LossAction::Preserve,
                "chat moderation.policy.input.mode",
            );
        }
        if let Some(mode) = output {
            ...
            report.record(
                ...
                LossAction::Preserve,
                "chat moderation.policy.output.mode",
            );
        }
