...
#[test]
fn proxy_dest_gemini_response_schema_reaches_chat() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        assert!(
            ...
            "dest Gemini responseSchema must reach Chat json_schema, got: {req}"
        );
    ...
    let profile = write_profile(
        &dir,
        "chat-from-gemini-schema.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-gemini-schema"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"contents":[{"role":"user","parts":[{"text":"city"}]}],"generationConfig":{"responseMimeType":"application/json","responseSchema":{"type":"object","properties":{"city":{"type":"string"}}}}}"#;
    let req = format!(
        "POST /v1beta/models/gemini-2.5-flash:generateContent HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        ...
        "dest Gemini schema follow-up must remap, got: {resp}"
    );
}
...
fn proxy_dest_responses_json_object_reaches_chat() {
    ...
    let upstream_thread = std::thread::spawn(move || {
        ...
        let req = String::from_utf8_lossy(&buf[..n]);
        assert!(
            req.contains("\"type\":\"json_object\"") && req.contains("response_format"),
            "dest Responses text.format json_object must reach Chat response_format, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"{\"ok\":true}"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
            body.len()
        );
    ...
    let profile = write_profile(
        &dir,
        "chat-from-responses-json-object.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-responses-json-object"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"model":"gpt-4o","input":"hi","text":{"format":{"type":"json_object"}}}"#;
    let req = format!(
        "POST /v1/responses HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        ...
        "dest Responses json_object follow-up must remap, got: {resp}"
    );
}

#[test]
fn proxy_dest_chat_json_object_reaches_chat() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        assert!(
            req.contains("\"type\":\"json_object\"") && req.contains("response_format"),
            "dest Chat response_format json_object must reach Chat, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"{\"ok\":true}"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-chat-json-object.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-chat-json-object"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"model":"gpt-4o","messages":[{"role":"user","content":"hi"}],"response_format":{"type":"json_object"}}"#;
    let req = format!(
        "POST /v1/chat/completions HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        resp.contains("HTTP/1.1 200") && resp.contains("ok"),
        "dest Chat json_object follow-up must remap, got: {resp}"
    );
}

#[test]
fn proxy_dest_chat_user_reaches_chat() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        let req = String::from_utf8_lossy(&buf[..n]);
        assert!(
            req.contains("\"user\":\"dest-user-42\""),
            "dest Chat user must reach Chat, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"hi"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-chat-user.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-chat-user"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body =
        r#"{"model":"gpt-4o","user":"dest-user-42","messages":[{"role":"user","content":"hi"}]}"#;
    let req = format!(
        "POST /v1/chat/completions HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        ...
        "dest Chat user follow-up must remap, got: {resp}"
    );
}

#[test]
fn proxy_dest_chat_verbosity_safety_metadata_reaches_chat() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        let req = String::from_utf8_lossy(&buf[..n]);
        assert!(
            req.contains("\"verbosity\":\"medium\""),
            "dest Chat verbosity must reach Chat, got: {req}"
        );
        assert!(
            req.contains("\"safety_identifier\":\"dest-safety-9\""),
            "dest Chat safety_identifier must reach Chat, got: {req}"
        );
        assert!(
            req.contains("\"metadata\":{\"ticket\":\"42\"}"),
            "dest Chat metadata must reach Chat, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"hi"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-chat-verbosity.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-chat-verbosity"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"model":"gpt-4o","verbosity":"medium","safety_identifier":"dest-safety-9","metadata":{"ticket":"42"},"messages":[{"role":"user","content":"hi"}]}"#;
    let req = format!(
        "POST /v1/chat/completions HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        resp.contains("HTTP/1.1 200") && resp.contains("hi"),
        "dest Chat verbosity/safety/metadata follow-up must remap, got: {resp}"
    );
}

#[test]
fn proxy_dest_messages_user_id_reaches_chat() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        assert!(
            req.contains("\"user\":\"dest-user-42\""),
            "dest Messages metadata.user_id must reach Chat user, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"hi"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-messages-user.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-messages-user"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"model":"claude-sonnet-4","max_tokens":16,"metadata":{"user_id":"dest-user-42"},"messages":[{"role":"user","content":"hi"}]}"#;
    let req = format!(
        "POST /v1/messages HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        resp.contains("HTTP/1.1 200") && resp.contains("hi"),
        "dest Messages user_id follow-up must remap, got: {resp}"
    );
    assert!(
        !dump.contains("sampling.user"),
        "--dump-loss must not Drop sampling.user, got: {dump}"
    );
}
...
#[test]
fn proxy_dest_messages_service_tier_reaches_chat() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        let req = String::from_utf8_lossy(&buf[..n]);
        assert!(
            req.contains("\"service_tier\":\"auto\""),
            "dest Messages service_tier must reach Chat service_tier, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"hi"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-messages-tier.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-messages-tier"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"model":"claude-sonnet-4","max_tokens":16,"service_tier":"auto","messages":[{"role":"user","content":"hi"}]}"#;
    let req = format!(
        "POST /v1/messages HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        resp.contains("HTTP/1.1 200") && resp.contains("hi"),
        "dest Messages service_tier follow-up must remap, got: {resp}"
    );
    assert!(
...
#[test]
fn proxy_dest_messages_disable_parallel_reaches_chat() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        let req = String::from_utf8_lossy(&buf[..n]);
        assert!(
            req.contains("\"parallel_tool_calls\":false"),
            "dest Messages disable_parallel_tool_use must reach Chat parallel_tool_calls false, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"hi"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-messages-parallel.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-messages-parallel"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"model":"claude-sonnet-4","max_tokens":16,"tool_choice":{"type":"auto","disable_parallel_tool_use":true},"messages":[{"role":"user","content":"hi"}]}"#;
    let req = format!(
        "POST /v1/messages HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        resp.contains("HTTP/1.1 200") && resp.contains("hi"),
        "dest Messages disable_parallel_tool_use follow-up must remap, got: {resp}"
    );
    assert!(
        !dump.contains("path: \"sampling.parallel_tool_calls\", action: Drop"),
        "--dump-loss must not Drop sampling.parallel_tool_calls, got: {dump}"
    );
}
...
#[test]
fn proxy_dest_converse_image_reaches_chat() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        let url = image
            ...
            .and_then(serde_json::Value::as_str)
            .unwrap_or("");
        let got = url.as_bytes();
        let mut expected = Vec::from(*b"data:");
        expected.extend_from_slice(b"image/png");
        expected.extend_from_slice(b";base64,");
        expected.extend_from_slice(b"iVBORw0KGgo");
        assert!(
            got.starts_with(&expected),
            "dest Converse image url prefix mismatch: got hex {} want hex {}",
            got.iter()
                .take(expected.len() + 8)
                .map(|b| format!("{b:02x}"))
                .collect::<String>(),
    ...
    let profile = write_profile(
        &dir,
        "chat-from-converse-image.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-converse-image"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"messages":[{"role":"user","content":[{"text":"see"},{"image":{"format":"png","source":{"bytes":"iVBORw0KGgo"}}}]}]}"#;
    let req = format!(
        "POST /model/amazon.nova-lite-v1:0/converse HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        resp.contains("HTTP/1.1 200") && resp.contains("ok"),
        "dest Converse image follow-up must remap, got: {resp}"
    );
    assert!(
        !dump.contains("part.image") && !dump.contains("converse image"),
        "--dump-loss must not Drop dest Converse image, got: {dump}"
    );
}

#[test]
fn proxy_dest_converse_audio_reaches_chat() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        let content = up
            ...
            .cloned()
            .unwrap_or_default();
        let audio = content.iter().find(|part| {
            part.get("type").and_then(serde_json::Value::as_str) == Some("input_audio")
        });
        let data = audio
            .and_then(|part| part.pointer("/input_audio/data"))
            .and_then(serde_json::Value::as_str);
        let format = audio
            .and_then(|part| part.pointer("/input_audio/format"))
            .and_then(serde_json::Value::as_str);
        assert_eq!(
            data,
            Some("SUQz"),
            "dest Converse audio data must reach Chat input_audio, got {up}"
        );
        assert_eq!(
            format,
            Some("mp3"),
            "dest Converse audio format must reach Chat input_audio, got {up}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"ok"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-converse-audio.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-converse-audio"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"messages":[{"role":"user","content":[{"text":"hear"},{"audio":{"format":"mp3","source":{"bytes":"SUQz"}}}]}]}"#;
    let req = format!(
        "POST /model/amazon.nova-lite-v1:0/converse HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    assert!(
        resp.contains("HTTP/1.1 200") && resp.contains("ok"),
        "dest Converse audio follow-up must remap, got: {resp}"
    );
    assert!(
        !dump.contains("part.audio"),
        "--dump-loss must not Drop part.audio, got: {dump}"
    );
}
...
fn proxy_dest_gemini_json_mime_reaches_chat() {
    ...
    let upstream_thread = std::thread::spawn(move || {
        ...
        assert!(
            req.contains("\"type\":\"json_object\"") && req.contains("response_format"),
            "dest Gemini responseMimeType application/json must reach Chat json_object, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"{\"ok\":true}"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-gemini-json-mime.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-gemini-json-mime"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"contents":[{"role":"user","parts":[{"text":"hi"}]}],"generationConfig":{"responseMimeType":"application/json"}}"#;
    let req = format!(
        "POST /v1beta/models/gemini-2.5-flash:generateContent HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
...
#[test]
fn proxy_dest_responses_unary_model_in_body() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        assert!(
            req.contains("\"model\":\"gpt-4o\""),
            "dest Responses model must reach Chat upstream, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"hi"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-responses-unary-model.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-responses-unary-model"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    upstream_thread.join().expect("upstream");
    assert!(
        resp.contains("\"model\":\"gpt-4o\""),
        "dest Responses unary JSON must keep dest model, got: {resp}"
    );
}

#[test]
fn proxy_dest_responses_stream_model_in_created() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        assert!(
            req.contains("\"model\":\"gpt-4o\""),
            "dest Responses stream model must reach Chat upstream, got: {req}"
        );
        let body = concat!(
            "data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"hi\"}}]}\n\n",
            "data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}]}\n\n",
    ...
    let profile = write_profile(
        &dir,
        "chat-from-responses-stream-model.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-responses-stream-model"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    assert!(
        resp.contains("\"model\":\"gpt-4o\""),
        "dest Responses stream must emit dest model, got: {resp}"
    );
}

#[test]
fn proxy_dest_responses_stream_text_item_done_keeps_text() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        assert!(
            ...
            "dest Responses stream text must reach Chat upstream, got: {req}"
        );
        let body = concat!(
            "data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"P\"}}]}\n\n",
            "data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"ong\"}}]}\n\n",
            "data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}]}\n\n",
            "data: [DONE]\n\n",
        );
        let resp = format!(
    ...
    let profile = write_profile(
        &dir,
        "chat-from-responses-stream-text.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-responses-stream-text"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let done_item = resp
        ...
        .unwrap_or("");
    assert!(
        done_item.contains("\"type\":\"message\"") && done_item.contains("Pong"),
        "dest Responses output_item.done must keep assembled text, got: {resp}"
    );
}

#[test]
fn proxy_dest_responses_stream_reasoning_item_done_keeps_summary() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        assert!(
            req.contains("\"stream\":true"),
            "dest Responses stream reasoning must reach Chat upstream, got: {req}"
        );
        let body = concat!(
    ...
    let profile = write_profile(
        &dir,
        "chat-from-responses-stream-reason.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-responses-stream-reason"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let _ = child.wait();
    upstream_thread.join().expect("upstream");
    let reason_done = resp
        .split("event: response.output_item.done")
        .nth(1)
        .unwrap_or("");
    assert!(
        reason_done.contains("\"type\":\"reasoning\"") && reason_done.contains("think"),
        "dest Responses reasoning output_item.done must keep the summary, got: {resp}"
    );
}

#[test]
fn proxy_dest_responses_tool_calls_status_is_completed() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        assert!(
            req.contains("get_weather"),
            "dest Responses tools must reach Chat upstream, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":null,"tool_calls":[{"id":"call_1","type":"function","function":{"name":"get_weather","arguments":"{\"city\":\"Paris\"}"}}]},"finish_reason":"tool_calls"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
            body.len()
        );
    ...
    let profile = write_profile(
        &dir,
        "chat-from-responses-tools-status.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-responses-tools-status"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"model":"gpt-4o","input":"weather Paris","tools":[{"type":"function","name":"get_weather","parameters":{"type":"object","properties":{"city":{"type":"string"}}}}]}"#;
    let req = format!(
        "POST /v1/responses HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    upstream_thread.join().expect("upstream");
    assert!(
        resp.contains("\"type\":\"function_call\"") && resp.contains("get_weather"),
        "dest Responses tools must remap function_call, got: {resp}"
    );
    assert!(
        resp.contains("\"status\":\"completed\""),
        "dest Responses tool turn must use official completed status, got: {resp}"
    );
    assert!(
        !resp.contains("\"status\":\"tool_calls\""),
        "dest Responses must not leak Chat tool_calls as status, got: {resp}"
    );
}

#[test]
fn proxy_dest_responses_stream_tool_item_done_keeps_call() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        let req = String::from_utf8_lossy(&buf[..n]);
        assert!(
            req.contains("\"stream\":true") && req.contains("get_weather"),
            "dest Responses stream tools must reach Chat upstream, got: {req}"
        );
        let body = concat!(
            "data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"call_1\",\"type\":\"function\",\"function\":{\"name\":\"get_weather\",\"arguments\":\"\"}}]}}]}\n\n",
            "data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{\"tool_calls\":[{\"index\":0,\"function\":{\"arguments\":\"{\\\"city\\\":\\\"Paris\\\"}\"}}]}}]}\n\n",
            "data: {\"id\":\"1\",\"object\":\"chat.completion.chunk\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"tool_calls\"}]}\n\n",
            "data: [DONE]\n\n",
        );
        let resp = format!(
    ...
    let profile = write_profile(
        &dir,
        "chat-from-responses-stream-tools.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-responses-stream-tools"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"model":"gpt-4o","stream":true,"input":"weather Paris","tools":[{"type":"function","name":"get_weather","parameters":{"type":"object","properties":{"city":{"type":"string"}}}}]}"#;
    let req = format!(
        "POST /v1/responses HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    upstream_thread.join().expect("upstream");
    assert!(
        resp.contains("event: response.output_item.done"),
        "dest Responses stream tools must emit output_item.done, got: {resp}"
    );
    let done_item = resp
        .split("event: response.output_item.done")
        .nth(1)
        .unwrap_or("");
    assert!(
        done_item.contains("\"name\":\"get_weather\"")
            && done_item.contains("Paris")
            && done_item.contains("\"call_id\":\"call_1\""),
        "dest Responses output_item.done must keep the function_call item, got: {resp}"
    );
}

#[test]
fn proxy_dest_gemini_function_response_matches_tool_call_id() {
    let upstream = TcpListener::bind("127.0.0.1:0").expect("upstream bind");
    let upstream_addr = upstream.local_addr().expect("addr");
    let upstream_thread = std::thread::spawn(move || {
        let (mut stream, _) = upstream.accept().expect("accept");
        ...
        let up: serde_json::Value =
            serde_json::from_str(&req[body_start..]).expect("upstream json");
        let call_id = up
        ...
        assert_eq!(
            call_id, result_id,
            "dest Gemini functionResponse must reuse the functionCall id, got: {req}"
        );
        let body = r#"{"id":"1","object":"chat.completion","choices":[{"index":0,"message":{"role":"assistant","content":"sunny"},"finish_reason":"stop"}]}"#;
        let resp = format!(
            "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
            body.len()
        );
    ...
    let profile = write_profile(
        &dir,
        "chat-from-gemini-tool-result.toml",
        &format!(
            r#"
schema_version = 1
id = "chat-from-gemini-tool-result"
wire = "chat-completions"
auth_scheme = "none"
base_url = "http://{upstream_addr}"
chat_path = "/v1/chat/completions"
"#
    ...
    let listen = read_listen_addr(child.stdout.as_mut().expect("stdout"));
    let body = r#"{"contents":[{"role":"user","parts":[{"text":"weather Paris"}]},{"role":"model","parts":[{"functionCall":{"name":"get_weather","args":{"city":"Paris"}}}]},{"role":"user","parts":[{"functionResponse":{"name":"get_weather","response":{"content":"sunny 22C"}}}]}],"tools":[{"functionDeclarations":[{"name":"get_weather","parameters":{"type":"object","properties":{"city":{"type":"string"}}}}]}]}"#;
    let req = format!(
        "POST /v1beta/models/gemini-2.5-flash:generateContent HTTP/1.1\r\nHost: 127.0.0.1\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
        body.len()
    );
    ...
    upstream_thread.join().expect("upstream");
    assert!(
        resp.contains("HTTP/1.1 200") && resp.contains("sunny"),
        "dest Gemini tool-result follow-up must remap, got: {resp}"
    );
}
